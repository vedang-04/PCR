...
def prepare_training(ret, batch):
    ...
    if cfg.poly_cls_branch:
        init.update({'neg_i_it_py': collect_training(batch['neg_i_it_py'], ct_01)})
    ...
    if 0:
        ...
        for k in range(n):
            a_k = a[k]
            b_k = b[k]
            ak_x = a_k[:, 0].cpu().numpy().tolist()
            ak_x.append(ak_x[0])
            ak_y = a_k[:, 1].cpu().numpy().tolist()
            ak_y.append(ak_y[0])
            bk_x = b_k[:, 0].cpu().numpy().tolist()
            bk_x.append(bk_x[0])
            bk_y = b_k[:, 1].cpu().numpy().tolist()
            bk_y.append(bk_y[0])
            plt.plot(ak_x, ak_y, 'y')
            plt.plot(bk_x, bk_y, 'g')
        plt.savefig('c.png')
        exit()

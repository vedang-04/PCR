...
class Dataset(data.Dataset):
    ...
    def prepare_detection(self, box, poly, ct_hm, cls_id, wh, ct_cls, ct_ind, ct_reg_pts=None, centerness=None,
                          rs_hm=None):
        ct_hm = ct_hm[cls_id]
    ...
    def prepare_shrunk_evolution(self, poly, extreme_point, img_init_shrunk_polys, can_init_shrunk_polys,
                                 img_gt_shrunk_polys, can_gt_shrunk_polys):
    ...
    def prepare_neg_evolution(self, poly, instance_polys, img_init_polys, can_init_polys, img_gt_polys, can_gt_polys,
                              inp_out_hw, img=None):
        ...
        for i in range(len(anno)):
            ...
            for j in range(len(instance_poly)):
                ...
                if h <= 1 or w <= 1:
                    continue
                if cfg.train.ct_reg:
                    decode_box = self.prepare_detection(bbox, poly, ct_hm, cls_id, wh, ct_cls, ct_ind, ct_reg,
                                                        rs_hm=rs_hm)
                    ...
                    self.prepare_init(bbox, extreme_point, i_it_4pys, c_it_4pys, i_gt_4pys, c_gt_4pys, output_h,
                                      output_w)

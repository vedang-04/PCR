...
class Evolution(nn.Module):
    def __init__(self):
        ...
        if cfg.csm_version == 'csm_v0':
            ...
            for i in range(self.iter):
                evolve_gcn = Snake(state_dim=128, feature_dim=contour_in_feat_channel + 2,
                                   conv_type=cfg.snake_conv_type)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
            final_evolve_gcn = Snake(state_dim=128, feature_dim=contour_in_feat_channel + 2,
                                     conv_type=cfg.snake_conv_type, snake_type='evolve_final')
            ...
            for i in range(self.iter):
                evolve_gcn = Snake(state_dim=128, feature_dim=contour_in_feat_channel + 2,
                                   conv_type=cfg.snake_conv_type)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
            final_loc_evolve = Snake(state_dim=128, feature_dim=contour_in_feat_channel + 2,
                                     conv_type=cfg.snake_conv_type)
            ...
            self.evolve_gcn = Snake(state_dim=128, feature_dim=contour_in_feat_channel + 2,
                                    conv_type=cfg.snake_conv_type)
            self.iter = cfg.evolve_iter
            for i in range(self.iter):
                if i != self.iter - 1:
    ...
    def forward(self, output, cnn_feature, batch=None):
        ...
        if batch is not None and 'test' not in batch['meta']:
            ...
            if cfg.use_otpg_flag:
                ex_pred, init_poly_pred = self.init_poly(self.init_gcn, cnn_feature, init['i_it_4py'], init['c_it_4py'],
                                                         init['4py_ind'])
            ...
            if cfg.csm_version == 'csm_v0':
                ...
                if cfg.poly_cls_branch:
                    ...
                    final_cls_net = None
                py_cls, py_pred, final_feat = self.loc_cls_head(cnn_feature, final_loc_evolve, final_cls_net, py_pred,
                                                                c_py_pred, init['py_ind'], batch)
                py_preds.append(py_pred)
                iter_py_cls = [py_cls]
            else:
                if cfg.poly_cls_branch:
                    iter_py_cls = []
                py_pred, final_feat = self.evolve_poly(self.evolve_gcn, cnn_feature, init['i_it_py'], init['c_it_py'],
                                                       init['py_ind'])
                py_preds = [py_pred]
                for i in range(self.iter):
                    py_pred = py_pred / snake_config.ro
                    ...
                    if (i == self.iter - 1) and cfg.poly_cls_branch:
                        ...
                        py_pred, final_feat = self.evolve_poly(evolve_gcn, cnn_feature, py_pred, c_py_pred,
                                                               init['py_ind'])
                    py_preds.append(py_pred)
        ...
        if not self.training:
            with torch.no_grad():
                ...
                if cfg.use_otpg_flag:
                    ex_pred, init_poly_pred = self.init_poly(self.init_gcn, cnn_feature, init['i_it_4py'],
                                                             init['c_it_4py'], init['ind'])
                ...
                if cfg.csm_version == 'csm_v0':
                    ...
                    if cfg.poly_cls_branch:
                        iter_py_cls_lst = []
                        py_cls, py, final_feat = self.evolve_poly(evolve_gcn, cnn_feature, py, c_py, init['ind'],
                                                                  cfg.poly_cls_branch)
                    ...
                    if cfg.poly_cls_branch:
                        final_cls_net = self.__getattr__('final_cls_net')
                    py_cls, py, final_feat = self.loc_cls_head(cnn_feature, final_loc_evolve, final_cls_net, py, c_py,
                                                               init['ind'])
                    pys.append(py / snake_config.ro)
                    ...
                    if cfg.poly_cls_branch:
                        iter_py_cls_lst = []
                    py, final_feat = self.evolve_poly(self.evolve_gcn, cnn_feature, evolve['i_it_py'],
                                                      evolve['c_it_py'], init['ind'])

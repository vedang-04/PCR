...
from lib.cocoapi.PythonAPI.pycocotools.mask import iou, merge, frPyObjects
...
def poly_rle_nms_fast(polys, scores, img_size, nms_thresh=0.3):
    ...
    rlePolys = frPyObjects(polys, _h, _w)
    mask_keeps = merge(rlePolys, nms_thresh)
...
def inference():
    ...
    for batch in tqdm.tqdm(dataset):
        ...
        if len(pys) == 0:
            ...
            bboxes_tmp = [data_utils.affine_transform(det[:4].reshape(-1, 2), trans_output_inv).flatten() for det in
                          detections]

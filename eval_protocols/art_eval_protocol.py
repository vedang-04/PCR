...
def curve_parse_gt_txt(filename):
    ...
    with open(filename.strip(), 'r') as f:
        ...
        for obj in gts:
            ...
            list_cors = cors[0].split(' ', )
            cors_num = len(list_cors[2:-2])
            obj_struct = {'name': str(list_cors[-1]), 'difficult': int(list_cors[1]),
                          'bbox': [int(i) for i in list_cors[2:-2]]}
            # class name
...
def load_det_set(det_dir, det_lst, cache_dir=None, conf_thresh=0.001):
    ...
    BB = [[float(z) for z in x[4:8]] for x in splitlines]
...
def eval_art(img_dir, gt_dir, det_dir, eval_res_dir, ovthresh=0.5, conf_thresh=0.001):
    ...
    if '.ipynb_checkpoints' in gt_lst:
        gt_lst = gt_lst[1:]
    if '.ipynb_checkpoints' in det_lst:
        det_lst = det_lst[1:]
    print("gt_lst_len:", len(gt_lst))
    print("det_lst_len:", len(det_lst))

    assert len(gt_lst) == len(det_lst), 'gt_file number ({})  v.s. det_file number ({})'.format(len(gt_lst),
                                                                                                len(det_lst))
    ...
    for d in range(nd):
        ...
        x1 = det_bbox[0]
        y1 = det_bbox[1]
        x2 = det_bbox[0] + det_bbox[2]
        y2 = det_bbox[1] + det_bbox[3]

        det_pts = ((x1, y1), (x1, y2), (x2, y1), (x2, y2))
...
if __name__ == '__main__':
    ...
    print("----------------------------------- Evaluating (iou_thresh={}) ----------------------------------".format(
        args.eval_iou_thresh))
    AP, Recall, Precision, F_score = eval_art(args.imgs_dir, args.gts_dir, args.dets_dir, args.eval_dir,
                                              args.eval_iou_thresh, conf_thresh=args.conf_thresh)

    print("Conf:{}, R:{}, P:{}, F:{}".format(args.conf_thresh, Recall, Precision, F_score))

    fid = open(args.eval_res_file, 'w')
    fid.write(
        'IOU(conf={:.2f})　| Precision:{}, Recall:{}, F_score:{}\n'.format(args.conf_thresh, Precision, Recall, F_score))
    fid.close()
